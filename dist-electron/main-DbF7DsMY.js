...
async function ensureStorageDirExists() {
  try {
    ...
    if (errorCode === "ENOENT") {
      try {
        await fs$1.mkdir(storageDir, { recursive: true });
...
async function readStore(fileName, defaultValue) {
  ...
  try {
    const fileContent = await fs$1.readFile(filePath, { encoding: "utf-8" });
    if (!fileContent) {
      return defaultValue;
    }
    try {
...
async function writeStore(fileName, data) {
  ...
  try {
    ...
    await fs$1.writeFile(filePath, fileContent, { encoding: "utf-8" });
...
async function fileFromPath(path2, ...args) {
  const { fileFromPath: _fileFromPath } = await import("./fileFromPath-CLXAFkhH.js");
...
function createWindow() {
  win = new BrowserWindow({
    ...
    webPreferences: {
      ...
      // nodeIntegration: false, // 默认关闭，更安全
    }
  });
