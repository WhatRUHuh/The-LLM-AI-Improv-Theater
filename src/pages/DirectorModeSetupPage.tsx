...
import { Select, Checkbox, Button, message, Spin, Typography, Card, Row, Col, theme } from 'antd'; // 移除了 Radio, Divider, RadioChangeEvent
...
const DirectorModeSetupPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        logger.info('加载导演模式设置页初始数据...'); // 中文注释
        ...
        logger.info('数据已加载:', { scriptsResult, charactersResult, servicesResult }); // 中文注释

        if (scriptsResult.success && Array.isArray(scriptsResult.data)) {
          setScripts(scriptsResult.data);
        } else {
          message.error(`加载剧本列表失败: ${scriptsResult.error || '数据格式错误'}`); // 中文注释
        }

        if (charactersResult.success && Array.isArray(charactersResult.data)) {
          setCharacters(charactersResult.data);
        } else {
          message.error(`加载角色列表失败: ${charactersResult.error || '数据格式错误'}`); // 中文注释
        }
        ...
        if (servicesResult.success && servicesResult.data) {
          ...
          const modelPromises = servicesResult.data.map(async (service) => {
            try {
              ...
              if (modelsResult.success && modelsResult.data) {
                ...
                logger.warn(`获取 ${service.providerName} 模型列表失败或无数据，使用默认模型: ${service.defaultModels.join(', ')}`); // 中文注释
                return { providerId: service.providerId, models: service.defaultModels };
              }
            } catch (modelError: unknown) {
              const errorMsg = modelError instanceof Error ? modelError.message : String(modelError);
              logger.error(`调用获取 ${service.providerName} 模型时出错: ${errorMsg}`); // 中文注释
              return { providerId: service.providerId, models: service.defaultModels };
            }
          });
          ...
          message.error(`加载 AI 服务商失败: ${servicesResult.error}`); // 中文注释
        }

      } catch (error: unknown) {
        const errorMsg = error instanceof Error ? error.message : String(error);
        message.error(`加载设置数据时出错: ${errorMsg}`); // 中文注释
      } finally {
        setLoading(false);
      }
    };
    ...
    loadData();
    if (restoredState) {
        logger.info('恢复内部状态:', restoredState); // 中文注释
    }
  ...
  useEffect(() => {
    ...
    // 注意：这里的 key 'directorModeSetup' 需要与 useLastVisited 中的定义匹配
  ...
  const handleStartChat = () => {
    ...
    if (!allAIConfigured) {
      message.error(`请为以下 AI 角色配置服务商和模型：${missingConfigChars.join(', ')}`); // 中文注释
      return;
    }
    ...
    const chatConfig = {
      ...
      aiConfigs: Object.fromEntries(aiConfigs.entries()), // 包含所有选中角色的配置
    };
