...
const RoleEditorPage: React.FC = () => {
      ...
      const loadRole = async () => {
        try {
          ...
          if (result.success && Array.isArray(result.data)) {
            const roleToEdit = result.data.find(role => role.id === roleId);
            if (roleToEdit) {
              setInitialValues(roleToEdit);
              form.setFieldsValue(roleToEdit); // 填充表单
            } else {
              message.error('未找到要编辑的角色！');
              navigate('/roles'); // 跳转回列表页
            }
          } else {
  ...
  }, [isEditMode, roleId, navigate, form]);

  // 处理表单提交 (保存)
  const handleFinish = async (values: Omit<AICharacter, 'id'>) => {
    setLoading(true);
    try {
      const allRolesResult = await window.electronAPI.readStore(rolesFileName, [] as AICharacter[]);
      if (!allRolesResult.success || !Array.isArray(allRolesResult.data)) {
         message.error(`加载现有角色列表失败: ${allRolesResult.error || '未知错误'}`);
         setLoading(false);
         return;
      }

      let updatedRoles: AICharacter[];
      const currentRoles = allRolesResult.data;

      if (isEditMode && roleId) {
        // 编辑模式
        updatedRoles = currentRoles.map(role =>
          role.id === roleId ? { ...initialValues, ...values, id: roleId } : role // 合并旧数据和新数据
        );
      } else {
        // 添加模式
      ...
      const saveResult = await window.electronAPI.writeStore(rolesFileName, updatedRoles);
      if (saveResult.success) {
        message.success(isEditMode ? '角色已更新' : '角色已添加');
        navigate('/roles'); // 保存成功后返回列表页
      } else {
